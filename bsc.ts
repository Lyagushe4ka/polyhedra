...
const CONTRACT = '0x9234f83473c03be04358afc3497d6293b2203288';
...
const provider = new JsonRpcProvider('https://binance.llamarpc.com');
...
async function main() {
  ...
  const response = await axios.get('https://pub-88646eee386a4ddb840cfb05e7a8d8a5.r2.dev/bsc_data/570.json');
  ...
  for (const key of keys) {
    ...
    if (!data[wallet.address]) {
      console.log('Missing proof for: ', wallet.address);
      continue;
    }

    const isClaimed = await contract.isClaimed(data[wallet.address].index);
